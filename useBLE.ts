...
export async function requestBlePermissions(): Promise<boolean> {
  ...
  try {
    ...
    if (apiLevel >= 31) {
      ...
      // Location may still be required on some devices or Android versions so I just request it to be sure
...
function scanAndConnect() {
  manager.startDeviceScan(null, null, (error, device) => {
    ...
    // Check if it is a device that I'm looking for based on advertisement data
    // or other criteria.
    if (device.name === 'Wahoo RPM Cadence' || device.name === 'RPM Cadence') {
      
      // Stop scanning as it's not necessary if I'm scanning for one device.
